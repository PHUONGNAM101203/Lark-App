        ...
        try {
            if (message.message_type === 'file') {
                ...
                if (file_name.toLowerCase().endsWith('.csv')) {
                    ...
                    // ✅ LẤY TOKEN ĐỘNG ĐỂ TRÁNH LỖI HẾT HẠN
                    ...
                    console.log(`🚀 Bắt đầu Vẽ Form, Style 13, Gộp Ô, Kẻ Bảng & Logo...`);
                    for (const r of rowsData) {
                        ...
                        // 🛠 2. GỘP Ô (Merge Cells) CHỐNG TRÀN CHỮ
                        const mergeRanges = [
                        ...
                        // 🛠 3. ĐỊNH DẠNG STYLE VÀ KẺ BẢNG (Kết hợp Code của sếp và Tính năng mới)
                        const borderLine = { style: "SOLID", color: "#000000" };
                        const stylePayload = {
                            data: [
                                // Ép Full Font 13 (Sử dụng Số nguyên 13 để không bị báo lỗi invalid fontSize)
                                { ranges: [`${targetId}!A1:F20`], style: { font: { fontSize: 13 } } },
                                // In đậm tên cty và Total
                                ...
                                // Thanh Header bôi nền xám, in đậm, căn giữa
                                { ranges: [`${targetId}!A16:F16`], style: { font: { bold: true, fontSize: 13 }, backColor: "#D9D9D9", hAlign: 2 } },
                                // Căn lề trái & lề trên cho cụm Thông tin khách hàng (Buyer -> Phone)
                        ...
                        // 🛠 4. CHÈN LOGO (Bê nguyên xi đoạn code chuẩn nhất của sếp vào đây)
                        try {
                            if (fs.existsSync('./public/logo.png')) {
                                const imgBuffer = fs.readFileSync('./public/logo.png');
                                // Sử dụng Blob chuẩn hệ thống
                                ...
                                const imgRes = await fetch(`https://open.larksuite.com/open-apis/sheets/v2/spreadsheets/${ssToken}/values_image`, {
                                    method: 'POST',
                                    headers: { 'Authorization': `Bearer ${tenantToken}` }, // Fetch tự xử lý Boundary
                                    body: form
                                });
